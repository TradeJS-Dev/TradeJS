...
def compute_ensemble_cutoffs(source_df: pd.DataFrame) -> list[int]:
    ...
    t1, t2 = np.quantile(ts_values, [0.5, 1.0])
...
def train_incremental_catboost(
    ...
    if ensemble:
        ...
        model_path = f'{model_base}.joblib'
        pipeline = Pipeline(
            steps=[
                ('preprocess', 'passthrough'),
                ('model', prod_models[0]),
...
def main() -> None:
    ...
    parser.add_argument(
        '--ensemble',
        action='store_true',
        help='Train 2 expanding-window models, eval on holdout, then retrain prod ensemble on all data',
    )
    ...
    if args.ensemble:
        ...
        eval_cutoffs = compute_ensemble_cutoffs(train_df)
        eval_models = []
