...
- Runtime evidence has one accepted composition contract: embedded deployment snapshot schema v2 and runtime lineage schema v3 on every evaluation, signal, trade, and persisted scope. Do not translate another runtime schema or reconstruct identity from `strategyConfigs`, mutable Redis, evaluation names, or current source metadata. Lineage-less `evaluationStatsBuckets` are debug telemetry only: never publish, sync, replay, or score them as immutable runtime evidence.
